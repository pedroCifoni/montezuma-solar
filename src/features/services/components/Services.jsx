...
const DiagnosticShuffler = ({ content }) => {
  ...
  return (
    <Card padding="p-8" className="min-h-[320px]">
      <h3 className="font-heading font-bold text-2xl text-primary mb-2">{content.title}</h3>
      <p className="text-textDark/70 font-body mb-8 md:text-sm lg:text-base">{content.description}</p>
      <div className="relative flex-1 flex flex-col justify-end perspective-1000">
        {items.map((item, i) => (
          <div 
            key={item} 
            className="absolute inset-x-0 bottom-0 bg-background rounded-2xl p-4 shadow-sm border border-primary/10 flex items-center gap-3 transition-all duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
            style={{ 
...
const TelemetryTypewriter = ({ content }) => {
  ...
  return (
    <Card padding="p-8" className="bg-secondary text-textLight min-h-[320px] shadow-[0_15px_30px_rgba(6,11,20,0.5)] hover:border-accent/30" highlight>
      ...
      <h3 className="font-heading font-bold text-2xl mb-2">{content.title}</h3>
      <p className="text-textLight/60 font-body mb-6 md:text-sm lg:text-base">{content.description}</p>
...
const Scheduler = ({ content }) => {
  return (
    <Card padding="p-8" className="min-h-[320px] group overflow-hidden">
      <h3 className="font-heading font-bold text-2xl text-primary mb-2">{content.title}</h3>
      <p className="text-textDark/70 font-body mb-8 md:text-sm lg:text-base">{content.description}</p>
